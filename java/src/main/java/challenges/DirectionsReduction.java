...
public class DirectionsReduction {
    ...
    public static String[] dirReduc(String[] arr) {
        ...
        for (String direction:arr) {
            string.append(direction).append(" ");
        }

        int previousLength = 0;
        while (previousLength != string.length() && string.length() > 5) {
            previousLength = string.length();
            string = new StringBuilder(string.toString().replaceAll("NORTH SOUTH |SOUTH NORTH |EAST WEST |WEST EAST ", ""));
        }
